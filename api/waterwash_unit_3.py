...
FROM unit3 ORDER BY time DESC LIMIT 1
...
"""
mean_rate_query_body = """SELECT mean("waterwash_rate") AS "mean_waterwash_rate" FROM unit3 WHERE time > now() - 2m """
...
while True:
    ...
    try:
            ...
            if df.index == my_data.index:
               ...
               # print("Pn: ", mownost_nagnetatelya.values)
                ...
                if  waterwash_check['status'] > waterwash:
                    ...
                    upload_whole_time_waterwash_alert = writer_client.write_points(pd_whole_time_waterwash_alert, 'unit3')
                ...
                try:
                    ...
                    upload_data_to_influx = writer_client.write_points(data_to_influx, ' unit3')

                    print("UNIT_3: Connected")


                except:

                    print("UNIT_3: Imitation model is not working")
                ...
                upload_waterwash_over_time = writer_client.write_points(waterwash_over_time, ' unit3')

                upload_power_e = power_e_writer_client.write_points(data_to_influx_power_e, 'unit3')
        ...
        print("UNIT_3: DATABASE NOT FOUND")
