...
try:
    ...
    url='http://flaskserver:5001/start-telebot',
...
def keyboard (message):
    ...
    try:
        response = requests.post(
            url='http://flaskserver:5001/all-users',
...
def main_menu(message):
    ...
    if message.text == "ШГН # 1":
        ...
        - Производительность: *{}%*
        - Доступность: *{}*.
        """.format(dict_data_3['last'], dict_data_3['last_2'], round(dict_data_3['last_1'],2), dict_data_3['last_3'])
        bot.send_message(message.chat.id, text_1, parse_mode='Markdown', reply_markup=markup_menu_3)
...
def main_menu(message):
    ...
    if message.text == "Да":
        ...
        try:
            response = requests.post(
            url='http://flaskserver:5001/unsubscribeall',
            data=json.dumps(data2), headers=header)
...
def main_menu(message):
    ...
    if message.text == "ШГН # 1":
        ...
        try:
            response = requests.post(
            url='http://flaskserver:5001/subscirbe',
            data=json.dumps(data4), headers=header)
        except:
        ...
        try:
            _ = requests.post(url='http://localhost:5001/subscirbe',
            data=json.dumps(data2), headers=header)
        except:
            print('Host can not connect, to subscribe a user, user wants to subscribe!')
            pass
        ...
        try:
            _ = requests.post(url='http://flaskserver:5001/subscirbe',
                              data=json.dumps(data2), headers=header)
        except:
            print('Host can not connect, to subscribe a user, user wants to subscribe!')
            pass
